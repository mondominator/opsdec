  ...
  try {
    ...
    const mostWatchedMovies = db.prepare(`
      SELECT
        ...
        h.media_type,
        (SELECT h2.thumb FROM history h2 WHERE h2.title = h.title AND h2.media_type = 'movie' AND h2.thumb IS NOT NULL ORDER BY h2.watched_at DESC LIMIT 1) as thumb,
    ...
    const mostWatchedEpisodes = db.prepare(`
      SELECT
        ...
        h.media_type,
        (SELECT h2.thumb FROM history h2 WHERE h2.grandparent_title = h.grandparent_title AND h2.media_type = 'episode' AND h2.thumb IS NOT NULL ORDER BY h2.watched_at DESC LIMIT 1) as thumb,
        COUNT(DISTINCT COALESCE(um.primary_username, h.username)) as unique_users,
        COUNT(DISTINCT h.title) as unique_episodes,
        COUNT(*) as plays,
        MAX(h.watched_at) as last_watched,
        SUM(CASE
          WHEN h.watched_at > strftime('%s', 'now', '-7 days') THEN 3
    ...
    // Audiobooks: Users first, then engagement (completions + hours + recency) as tiebreaker
    // This ensures books with more listeners always rank higher
    const mostWatchedAudiobooks = db.prepare(`
      SELECT
        h.title,
        MAX(h.parent_title) as parent_title,
        h.media_type,
        (SELECT h2.thumb FROM history h2 WHERE h2.title = h.title AND h2.media_type IN ('audiobook', 'track', 'book') AND h2.thumb IS NOT NULL ORDER BY h2.watched_at DESC LIMIT 1) as thumb,
        MAX(h.media_id) as media_id,
        COUNT(DISTINCT COALESCE(um.primary_username, h.username)) as unique_users,
